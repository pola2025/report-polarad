...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!airtableConfig) {
      return NextResponse.json(
        { error: `클라이언트 ${client.slug}의 Airtable 설정이 없습니다.` },
        { status: 400 },
      );
    }
    ...
    if (rows.length < 2) {
      return NextResponse.json(
        { error: "CSV 파일이 비어있거나 형식이 잘못되었습니다." },
        { status: 400 },
      );
    }
    ...
    let hasValidHeader = expectedHeaders.some((h) =>
      header.some((col) => col.includes(h)),
    );

    // 첫 번째 행이 헤더가 아니면 두 번째 행 확인 (네이버 월간리포트 제목 줄 건너뛰기)
    if (!hasValidHeader && rows.length > 1) {
      header = rows[1];
      hasValidHeader = expectedHeaders.some((h) =>
        header.some((col) => col.includes(h)),
      );
      if (hasValidHeader) {
        headerRowIndex = 1;
    ...
    if (!hasValidHeader) {
      return NextResponse.json(
        {
          error:
            "네이버 광고 CSV 형식이 아닙니다. 필수 컬럼: 일별, 검색어, 노출수, 클릭수",
    ...
    const dataRows = rows
      ...
      .filter((row) => row.length >= 4 && row[dateIdx]);
    ...
    // 기존 레코드 삭제 (10개씩 배치, 5개 동시 실행)
    if (allExistingIds.length > 0) {
      const deleteBatches: string[][] = [];
      for (let i = 0; i < allExistingIds.length; i += 10) {
        deleteBatches.push(allExistingIds.slice(i, i + 10));
      }
      for (let i = 0; i < deleteBatches.length; i += 5) {
        const concurrent = deleteBatches.slice(i, i + 5);
        await Promise.all(
          concurrent.map((batch) => {
            const deleteParams = batch
              ...
              .join("&");
            return fetch(
              `https://api.airtable.com/v0/${airtableConfig.baseId}/${airtableConfig.tableId}?${deleteParams}`,
              {
                method: "DELETE",
                headers: { Authorization: `Bearer ${AIRTABLE_TOKEN}` },
              },
    ...
    // 2. 새 레코드 삽입 (10개씩 배치, 5개 동시 실행)
    const insertBatches: (typeof airtableRecords)[] = [];
    for (let i = 0; i < airtableRecords.length; i += 10) {
      insertBatches.push(airtableRecords.slice(i, i + 10));
    }
    for (let i = 0; i < insertBatches.length; i += 5) {
      const concurrent = insertBatches.slice(i, i + 5);
      ...
      if (failed) {
        console.error("Airtable insert error:", failed.error);
        return NextResponse.json(
    ...
    // 키워드별 통계
    const keywordStats = records.reduce(
      (acc, r) => {
        if (!acc[r.keyword]) {
