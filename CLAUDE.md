...
### 🚫 클라이언트별 지표 표시 규칙 (CRITICAL - 절대 규칙)

**meta_metric_type에 따라 표시 지표가 완전히 다름!**

| 타입 | 클라이언트 | 표시 지표 | 숨김 지표 |
|------|-----------|----------|----------|
| `video` | H.E.A 판교 | 노출, 클릭, 지출, **영상조회수**, **평균시청시간** | ❌ 리드(leads), ❌ 리드당비용(CPL) |
| `lead` | 나라똔 | 노출, 클릭, 지출, **리드**, **리드당비용(CPL)** | - |

**H.E.A 판교(video 타입)에서 절대 표시 금지:**
- ❌ 리드 수 (leads)
...
**H.E.A 판교에서 반드시 표시:**
- ✅ 영상 조회수 (video_views)
- ✅ 평균 영상 시청시간 (avg_watch_time)
- ✅ 노출수, 클릭수, CTR, CPC, 지출액

**분기 기준**: `clientInfo?.metaMetricType === 'video'` 또는 `meta_metric_type === 'video'`
